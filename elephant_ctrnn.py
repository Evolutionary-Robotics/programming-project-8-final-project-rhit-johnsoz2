...
class CTRNN:
    ...
    def step(self, dt, fitness):
        fitness_scale = fitness + 1

        net_input = self.inputs + np.dot(self.weights.T, self.outputs)
        net_input *= fitness_scale

        self.states += dt * (self.inv_time_constants * (-self.states + net_input))
        self.outputs = sigmoid(self.states + self.biases)

        self.outputs = np.clip(self.outputs, 0, 2)

        frequency_scale = 10
        stride_amplitude = 0.8
        phase_offset = np.linspace(0, np.pi, self.size)  # Offset for each leg
        self.outputs += stride_amplitude * np.sin(2 * np.pi * (self.time_step * 0.1 * frequency_scale) + phase_offset)  # Create wave pattern

        self.outputs = np.clip(self.outputs, 0, 1.5)
        self.time_step += dt
